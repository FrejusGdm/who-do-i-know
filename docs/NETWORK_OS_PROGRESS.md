...
### Password release validation

- Commits: `48d90ce` (private error handling) and `a23ba14` (owner setup/password auth). Both were fast-forwarded to remote `main` with explicit owner authorization.
- Validation: 30 unit tests and 49 disposable PostgreSQL integration tests passed; lint and type checking passed. The focused browser test created an owner credential, removed the invitation fragment, signed in through same-origin BetterAuth and reached private pages. The broader browser rerun was interrupted by local disk exhaustion after its first core workflow passed; the previous release's nine browser checks remain historical evidence, not a fresh full-suite pass.
- The local production build compiled and generated pages but failed while copying standalone output because the disk was full. This exposed Next tracing of local mailbox archives; explicit archive/private-directory exclusions were added. Only generated `.next` output was removed; original personal files and archives were preserved. The production image uses an explicit Git-source ZIP plus Docker build allowlist, so local archives and environment files are absent from the AWS source.
- AWS CodeBuild completed successfully for image `a23ba149ff616319c7b39d7c927c7121cc225e81`, digest `sha256:58124d3ba9b4232a105f596c72a18b836099c4fd5a9fdbfa0b8ae048fb500314`. The image scan still reports the previously documented OS-package exceptions: 3 critical, 11 high and 5 medium findings; this is not a vulnerability-free image.
- Migration 0013 was applied only to the isolated deployment database after verifying the complete existing migration history. The confirmed owner identity was configured privately in AWS Secrets Manager. No owner email, setup token or password was included in commits.
- Live HTTPS verification: `/login`, `/setup` and `/api/health` returned 200. Setup returns `Cache-Control: private, no-store` and `Referrer-Policy: no-referrer`; cross-origin setup returned 403, disabled public signup/social login returned 404, and a synthetic invalid login and unauthenticated private API access returned 401. The owner invitation was delivered through a private channel, not this record.
- CloudFormation reached `UPDATE_COMPLETE`; both replacement services use task-definition revision 4. The release changes only web/worker task definitions and preserves the existing network, URL and database location. Mentor mailbox import remains pending.
