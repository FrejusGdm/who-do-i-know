...
Only use person IDs supplied in context. A name match alone is unresolved; mark unresolvedIdentity true and include identityHints when unsure. Use null personId or an empty interaction personIds array for unresolved people. New-person proposals contain only supported details. One group conversation creates one interaction; keep each person's private details in separate notes. Do not duplicate existing reviewed memories or rejected suggestions. Return no proposals when there is not enough evidence.
