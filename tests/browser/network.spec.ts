...
test('conversation preferences retain private choices across retries, stale tabs and mobile edits', async ({ page, context }) => {
  page.setDefaultTimeout(30_000);
  const person = await createPerson(owner, { name: 'Conversation preferences browser fixture', relationshipType: 'mentor' });
  const endpoint = `${origin}/api/people/${person.id}/preferences`;
  await page.goto(`/people/${person.id}`);
  const section = page.getByRole('region', { name: 'Conversation preferences', exact: true });
  await section.getByRole('button', { name: 'Edit preferences', exact: true }).click();
  await section.getByLabel('What you want to maintain', { exact: true }).fill('Keep our synthetic mentorship close');
  await section.getByLabel('Topics that feel natural', { exact: true }).fill('Life in China and AI safety ideas');
  await section.getByLabel('Topics to avoid in drafts', { exact: true }).fill('Private family matters');
  await section.getByLabel('Photo with a caption', { exact: true }).check();
  await section.getByLabel('A question', { exact: true }).check();
  await section.getByLabel('Preferred language (optional)', { exact: true }).fill('Français');
  await page.route(endpoint, async route => { await route.fetch(); await route.abort('failed'); }, { times: 1 });
  await section.getByRole('button', { name: 'Save preferences', exact: true }).click();
  await expect(section.getByRole('alert')).toBeVisible();
  await expect(section.getByLabel('Topics that feel natural', { exact: true })).toHaveValue('Life in China and AI safety ideas');
  await section.getByRole('button', { name: 'Retry preferences', exact: true }).click();
  await expect(section.getByRole('button', { name: 'Edit preferences', exact: true })).toBeVisible();
  await expect(section.getByText('Keep our synthetic mentorship close', { exact: true })).toBeVisible();
  await captureBoth(page, 'preferences');
  const other = await context.newPage(); other.setDefaultTimeout(30_000);
  await other.goto(`/people/${person.id}`);
  const stale = other.getByRole('region', { name: 'Conversation preferences', exact: true });
  await stale.getByRole('button', { name: 'Edit preferences', exact: true }).click();
  await stale.getByLabel('Topics that feel natural', { exact: true }).fill('An older tab edit');
  await section.getByRole('button', { name: 'Edit preferences', exact: true }).click();
  await section.getByLabel('Topics that feel natural', { exact: true }).fill('A newer confirmed topic');
  await section.getByRole('button', { name: 'Save preferences', exact: true }).click();
  await expect(section.getByRole('button', { name: 'Edit preferences', exact: true })).toBeVisible();
  await expect(section.getByText('A newer confirmed topic', { exact: true })).toBeVisible();
  await stale.getByRole('button', { name: 'Save preferences', exact: true }).click();
  await expect(stale.getByRole('alert')).toContainText('These preferences changed');
  await expect(stale.getByLabel('Topics that feel natural', { exact: true })).toHaveValue('An older tab edit');
  await stale.getByRole('button', { name: 'Reload saved preferences', exact: true }).click();
  await expect(stale.getByRole('button', { name: 'Edit preferences', exact: true })).toBeVisible();
  await expect(stale.getByText('A newer confirmed topic', { exact: true })).toBeVisible();
  await other.close();
  await page.setViewportSize({ width: 390, height: 844 });
  await section.getByRole('button', { name: 'Edit preferences', exact: true }).click();
  await section.getByLabel('Preferred language (optional)', { exact: true }).fill('');
  await section.getByLabel('Photo with a caption', { exact: true }).uncheck();
  await section.getByRole('button', { name: 'Save preferences', exact: true }).click();
  await expect(section.getByRole('button', { name: 'Edit preferences', exact: true })).toBeVisible();
  await page.reload();
  await expect(section.getByText('Français', { exact: true })).toHaveCount(0);
  await expect(section.getByText('A question', { exact: true })).toBeVisible();
  await expect(section.getByText('Private family matters', { exact: true })).toBeVisible();
  const stored = await context.request.get(endpoint); expect(stored.status()).toBe(200);
  expect(stored.headers()['cache-control']).toBe('private, no-store');
  const data = { ...(await stored.json()).preferences, requestKey: randomUUID() };
  expect((await context.request.put(endpoint, { data })).status()).toBe(403);
  expect((await context.request.get(`${origin}/api/people/${foreignId}/preferences`)).status()).toBe(404);
  expect((await context.request.put(`${origin}/api/people/${foreignId}/preferences`, { headers: { Origin: origin }, data })).status()).toBe(404);
  expect((await context.request.put(endpoint, { headers: { Origin: origin }, data: { ...data, draftExclusions: 'x'.repeat(2001) } })).status()).toBe(400);
  expect((await context.request.get(endpoint, { headers: { Cookie: 'better-auth.session_token=forged' } })).status()).toBe(401);
  await db.update(people).set({ archivedAt: new Date() }).where(eq(people.id, person.id));
  await page.reload();
  await expect(section.getByRole('button', { name: 'Edit preferences', exact: true })).toHaveCount(0);
  await expect(section.getByText('A newer confirmed topic', { exact: true })).toBeVisible();
});

test('personal updates stay private by default, preserve explicit audiences, and recover deletion acknowledgments', async ({ page, context }) => {
  page.setDefaultTimeout(30_000);
  const { createCircle } = await import('../../src/lib/network/store');
  const person = await createPerson(owner, { name: `Update recipient ${randomUUID().slice(0, 6)}` });
  const circle = await createCircle(owner, { name: `Update circle ${randomUUID().slice(0, 6)}` });
  await page.goto('/updates');
  const composer = page.getByRole('region', { name: 'Add a personal update', exact: true });
  await composer.getByRole('button', { name: 'Add an update', exact: true }).click();
  await composer.getByLabel('Update title', { exact: true }).fill('A synthetic chapter');
  await composer.getByLabel('What would you like to remember?', { exact: true }).fill('A thought to share when the time feels right.');
  await composer.getByLabel('Date (optional)', { exact: true }).fill('2026-09-06');
  await expect(composer.getByText('Private — no draft audience selected', { exact: true })).toBeVisible();
  await page.route(`${origin}/api/updates`, async route => { await route.fetch(); await route.abort('failed'); }, { times: 1 });
  await composer.getByRole('button', { name: 'Save update', exact: true }).click();
  await expect(composer.getByRole('alert')).toBeVisible();
  await composer.getByRole('button', { name: 'Retry update', exact: true }).click();
  await expect(composer.getByRole('button', { name: 'Add an update', exact: true })).toBeVisible();
  let card = page.getByRole('article', { name: 'A synthetic chapter', exact: true });
  await expect(card).toHaveCount(1);
  await expect(card.getByText('Private — no draft audience selected', { exact: true })).toBeVisible();
  await card.getByRole('button', { name: 'Edit update', exact: true }).click();
  await card.getByLabel('Update title', { exact: true }).fill('A revised synthetic chapter');
  await card.getByText('Who may receive this in a draft?', { exact: true }).click();
  await card.getByLabel('Search by name', { exact: true }).fill(person.name);
  await card.getByRole('button', { name: 'Find', exact: true }).click();
  await card.getByRole('checkbox', { name: new RegExp(person.name) }).check();
  await card.getByRole('checkbox', { name: circle.name, exact: true }).check();
  await card.getByRole('button', { name: 'Save update', exact: true }).click();
  card = page.getByRole('article', { name: 'A revised synthetic chapter', exact: true });
  await expect(card.getByRole('button', { name: 'Edit update', exact: true })).toBeVisible();
  await expect(card.getByText(/Allowed in drafts for:/)).toContainText(person.name);
  await expect(card.getByText(/Allowed in drafts for:/)).toContainText(circle.name);
  await captureBoth(page, 'updates');
  await page.reload();
  await expect(card.getByText(/Allowed in drafts for:/)).toContainText(person.name);
  const response = await context.request.get(`${origin}/api/updates`);
  expect(response.status()).toBe(200); expect(response.headers()['cache-control']).toBe('private, no-store');
  const saved = (await response.json()).updates.find((row: { title: string }) => row.title === 'A revised synthetic chapter');
  expect(saved.allowedPersonIds).toEqual([person.id]); expect(saved.allowedCircleIds).toEqual([circle.id]);
  const endpoint = `${origin}/api/updates/${saved.id}`;
  expect((await context.request.patch(endpoint, { data: { requestKey: randomUUID(), revision: saved.revision, action: 'remove' } })).status()).toBe(403);
  expect((await context.request.post(`${origin}/api/updates`, { headers: { Origin: origin }, data: { requestKey: randomUUID(), title: 'Foreign audience', body: 'Rejected', allowedPersonIds: [foreignId] } })).status()).toBe(404);
  expect((await context.request.patch(`${origin}/api/updates/${randomUUID()}`, { headers: { Origin: origin }, data: { requestKey: randomUUID(), revision: 1, action: 'remove' } })).status()).toBe(404);
  expect((await context.request.get(`${origin}/api/updates`, { headers: { Cookie: 'better-auth.session_token=forged' } })).status()).toBe(401);
  expect((await context.request.post(`${origin}/api/updates`, { headers: { Origin: origin }, data: { requestKey: randomUUID(), title: 'Oversize', body: 'x'.repeat(4001) } })).status()).toBe(400);
  await page.setViewportSize({ width: 390, height: 844 });
  await card.getByRole('button', { name: 'Remove update', exact: true }).click();
  const dialog = page.getByRole('alertdialog');
  await expect(dialog).toContainText('Original interview words remain');
  await page.route(endpoint, async route => { await route.fetch(); await route.abort('failed'); }, { times: 1 });
  await dialog.getByRole('button', { name: 'Confirm removal', exact: true }).click();
  await expect(dialog.getByRole('alert')).toBeVisible();
  await dialog.getByRole('button', { name: 'Retry removal', exact: true }).click();
